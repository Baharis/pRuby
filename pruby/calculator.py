...
class PressureCalculator:
    ...
    def set_routine(self, reading='', backfitting='', peakfitting='',
        ...
        if reading:
            self.reading_routine = reading_routine_manager.select(reading)
        if backfitting:
            self.backfitting_routine = backfitting_routine_manager.select(backfitting)
        if peakfitting:
            self.peakfitting_routine = peakfitting_routine_manager.select(peakfitting)
        if correcting:
            self.correcting_routine = correcting_routine_manager.select(correcting)
        if translating:
            self.translating_routine = translating_routine_manager.select(translating)
        if drawing:
            self.drawing_routine = drawing_routine_manager.select(drawing)
